...
function ResourceAllocationChart({ teamMembers, tasks, projects }: { teamMembers: any[], tasks: any[], projects: any[] }) {
  const gold = '#E8B84B'; const goldDim = '#C9993A'
  const border = 'rgba(201,153,58,0.2)'; const textDim = '#A8C0DC'; const textMid = '#C8DCF4'; const textBright = '#F0F6FF'

  if (teamMembers.length === 0) return null

  const maxTasks = Math.max(...teamMembers.map((m: any) => tasks.filter((t: any) => t.owner === m.name && t.status !== 'done').length), 1)
...
function WorkflowView({ tasks, projects, milestones, teamMembers, user, supabase, isMobile, onCreated, gold, goldDim, border, textBright, textDim, s }: any) {
  const STAGES = ['Drafted', 'Submitted', 'Reviewed', 'Approved', 'Executed']
...
function MilestoneTrendChart({ milestones, project }: { milestones: any[], project: any }) {
  const goldDim = '#C9993A'; const navy = '#050D1A'
  const border = 'rgba(201,153,58,0.2)'; const textDim = '#A8C0DC'; const textMid = '#C8DCF4'; const textBright = '#F0F6FF'
  ...
  // Drift = days between project end date and milestone due date (positive = slipping past project end)
  const drift = (m: any) => {
    if (!project?.end_date || !m.due_date) return null
    const d = Math.ceil((new Date(m.due_date).getTime() - new Date(project.end_date).getTime()) / 86400000)
    return d
  }
  ...
  return (
          ...
          return (
            <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px', minHeight: '32px' }}>
              ...
              <div style={{ flex: 1, position: 'relative', height: '24px', background: 'rgba(255,255,255,0.03)', borderRadius: '3px', overflow: 'hidden' }}>
                {/* Filled bar up to milestone date */}
    ...
    <MilestoneTrendChart milestones={milestones.filter((m: Milestone) => m.project_id === (projects.find((p: Project) => p.id === selectedProjectId) || projects[0])?.id)} project={project} />
    </div>
  )
}
